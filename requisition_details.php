			...
			<div class="card">
				...
				<form action="data/create_requisition.php?deliver" method="POST">
					<div class="card-body">
						<?php
							$id = $_GET['id'];
							$sql = "SELECT requisition.req_no, (requisition.employee_name) as nme, requisition.req_type, requisition.project_name, requisition.user_id, user_accounts.employee_name, division.division_name, department.department_name FROM requisition,division,department, user_accounts WHERE requisition.division_id=division.division_id AND  requisition.department_id=department.department_id AND requisition.purchaser_id = user_accounts.employee_id AND requisition.id='$id'";
							...
							while ($reqInfo = mysqli_fetch_assoc($result)) {?>
								<div class="row">
									...
									<div class="col-md-2">
										...
										<input type="hidden" name="delivered_by" value="<?php echo $_SESSION['employee_id']; ?>">
									</div>
									<div class="col-md-2">
								...
								<div class="row">
									<div class="col-md-2"></div>
									<div class="col-md-2"><label>Purchaser Name:</label></div>
									<div class="col-md-2"><?php echo $reqInfo['employee_name']; ?></div>
									<div class="col-md-2">
									...
									<div class="col-md-2">
                                        ...
                                        $sql_prj_name = "SELECT requisition.*, projects.* FROM requisition, projects WHERE projects.id= requisition.project_name AND requisition.id = $id";
                                        $project_name = '';
								...
								<div class="row">
									...
									<!-- vendor info add -->
											...
											<hr>
												...
												<hr>
												<?php
													$sql = "SELECT  requisition.*, purchase.* FROM requisition, purchase WHERE requisition.req_no = purchase.pr_no AND requisition.id = $id";
													$result = mysqli_query($conn, $sql);
													while ($row_pur_id = mysqli_fetch_assoc($result)) {

														$purname= $row_pur_id['purchaser_id'];
														$req_no= $row_pur_id['req_no'];

                                                        $sql_name = "SELECT * FROM `user_accounts` WHERE `employee_id`='$purname'";
                                                        $employee_name = '';
                                                        $result_name = mysqli_query($conn, $sql_name);
                                                        $count_emp = mysqli_num_rows($result_name);
												...
												<div class="row">
													...
													<?php ?>
													<div class="col-md-2">
														<input type="text" class="form-control" name="purname" value="<?php if(isset($purname)){ echo $purname==''?'':$purname;} ?>">
													</div>
													<div class="col-md-2">
														<label>Purchaser ID-</label>
													</div>
													<div class="col-md-2">
														<input class="form-control" type="text" name="employ_name" value="<?php if(isset($employ_name)){ echo $employ_name==''?'':$employ_name;} ?>">
													</div>
													<div class="col-md-2"></div>
												</div>
												<hr>
												<div class="row">
													...
													<div class="col-md-2">
									<?php
										if(isset($req_no) && $req_no != ''){
											$sql_vendor = "SELECT * FROM `store` WHERE `req_number` = '$req_no' and `record_type`='Out'";
	                                        ...
	                                        if ($count_vendor > 0){
	                                            ...
	                                            {
	                                            	$vendor = $row_vendor["vendor"];
	                                            	$bill_no = $row_vendor["bill_no"];
	                                            	$st_rcvr_name = $row_vendor["st_rcvr_name"];
	                                            	$chalan_no = $row_vendor["chalan_no"];
	                                            	$st_rcvr_phn = $row_vendor["st_rcvr_phn"];
	                                            	$delivery_date = $row_vendor["delivery_date"];
	                                            	$st_address = $row_vendor["st_address"];
	                                            	$other_info = $row_vendor["other_info"];
	                                            	$site_name = $row_vendor["site_name"];
	                                            	$bill_no = $row_vendor["bill_no"];
	                                            }
	                                    	}
										}
	                                ?>
														<input type="text" class="form-control" name="vendor" value="<?php if(isset($vendor)){ echo $vendor==''?'':$vendor;} ?>" >
													</div>
													<div class="col-md-2">
	                                    		...
	                                    		echo '<input type="text" class="form-control" name="gate_pass_no" value="" placeholder="Enter Gate Pass">';
												?>
													</div>
													<div class="col-md-2"></div>
												</div>
												...
												if (isset($site_name))
	                                            {
	                                                echo '<input type="text" class="form-control" name="site_name" value="'.$site_name.'" >';
	                                    		}else{
	                                    		echo '<input type="text" class="form-control" name="site_name" value="" placeholder="Enter Site Name">';
	                                    		}
												?>
													</div>
													<div class="col-md-2">
												...
												<?php
												if (isset($bill_no)){
	                                                echo '<input type="text" class="form-control" name="bill_no" value="'.$bill_no.'" >';
	                                    		}else{
	                                    		echo '<input type="text" class="form-control" name="bill_no" value="" placeholder="Enter Bill No">';
	                                    		}
												?>
													</div>
													<div class="col-md-2"></div>
												...
												<div class="row">
													...
													<div class="col-md-2">
												<?php
												if (isset($st_rcvr_name)){
	                                                echo '<input type="text" class="form-control" name="st_rcvr_name" value="'.$st_rcvr_name.'" >';
	                                    		}else{
	                                    		echo '<input type="text" class="form-control" name="st_rcvr_name" value="" placeholder="Enter Receiver Name" required>';
	                                    		}
												?>
													</div>
													<div class="col-md-2">
												...
												<?php
												if (isset($chalan_no)){
	                                                echo '<input type="text" class="form-control" name="chalan_no" value="'.$chalan_no.'" >';
	                                    		}else{
	                                    		echo '<input type="text" class="form-control" name="chalan_no" value="" placeholder="Enter Chalan No" required>';
	                                    		}
												?>
													</div>
													<div class="col-md-2"></div>
												...
												<div class="row">
													...
													<div class="col-md-2">
												<?php
												if (isset($st_rcvr_phn)){
	                                                echo '<input type="text" class="form-control" name="st_rcvr_phn" value="'.$st_rcvr_phn.'" >';
	                                    		}else{
	                                    		echo '<input type="text" class="form-control" name="st_rcvr_phn" value="" placeholder="Enter Receiver Phone no" required>';
	                                    		}
												?>
													</div>
													<div class="col-md-2">
												...
												if (isset($delivery_date)){
	                                                echo '<input type="text" class="form-control" name="delivery_date" value="'.$delivery_date.'" >';
	                                    		}else{
	                                    		echo '<input type="text" class="form-control" name="delivery_date" value="" placeholder="YYYY-MM-DD"  required>';
	                                    		}
												?>
													</div>
													<div class="col-md-2"></div>
												...
												<?php
												if (isset($st_address)){
	                                                echo '<input type="text" class="form-control" name="st_address" value="'.$st_address.'" >';
	                                    		}else{
	                                    		echo '<input type="text" class="form-control" name="st_address" value="" placeholder="Enter Site Address" required>';
	                                    		}
												?>
													</div>
													<div class="col-md-2">
													...
													<div class="col-md-2">
														<?php
												if (isset($other_info)){
	                                                echo '<textarea class="form-control" name="other_info">'.$other_info.'</textarea>';
	                                    		}else{
	                                    		echo '<textarea class="form-control" name="other_info"></textarea>';
	                                    		}
												?>
													</div>
													<div class="col-md-2"></div>
