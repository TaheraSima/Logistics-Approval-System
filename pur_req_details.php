					...
					<div class="card-body">
						<?php 
							...
							while ($reqInfo = mysqli_fetch_assoc($result)) {
								...
								<div class="row">
									<div class="col-md-2"></div>
									<div class="col-md-2"><label>Checker name : </label></div>
									<div class="col-md-2">
										<?php 
											$chId=$reqInfo['checker_id'];
											$cName = "SELECT * FROM user_accounts WHERE employee_id = $chId ";
											$resultc = mysqli_query($conn, $cName);
											while ($checkerName = mysqli_fetch_assoc($resultc)) {
											echo $checkerName['employee_name']; 
										}
										?>
									</div>
									<div class="col-md-2">
                                       ...
                                       $sql_prj_name = "SELECT requisition.*, projects.* FROM requisition, projects WHERE projects.id= requisition.project_name AND requisition.id = $id";
                                        ...
                                        while ($rowprj_name = mysqli_fetch_assoc($resultproj_name)) {
                                        	if ($rowprj_name['name'] !="") {
                                        		...
                                        		<input type="hidden" name="name" class="form-control name" id="name" value="$project_name" >
                                        	<?php }
                                        	else{
                                        		$project_name = "";
                                        	?>                                  		
                                        		<input type="hidden" name="name" class="form-control name" id="name" value="" >
                                        	<?php
                                        	}
								...
								<div class="row">
									...
									<?php 
										...
										?>
									<div class="col-md-2"><label><?php echo $cname ; ?> </label></div>
								...
								<div class="row">
									...
									<div class="col-md-4">
										<?php
											$sql = "SELECT * FROM `purchase` WHERE `pr_no` = '$reqInfo[req_no]'";
											$result = mysqli_query($conn,$sql);
	                                        ...
	                                        if ($count > 0){
	                                            ...
	                                            {	                                            	
	                                                echo '<input type="text" class="form-control" name="supplier_name" value="'.$row["supplier_name"].'" >';
	                                            }
	                                    	}else{
	                                    		echo '<input type="text" class="form-control" name="supplier_name" value="" placeholder="Enter Supplier Name" required>';
	                                    	}
	                                    ?>
	                    ...
	                    <?php
	                        ...
	                        $sql = "SELECT requisition_details.*, item_info.item_id, item_info.item_name FROM  requisition_details,item_info WHERE requisition_details.item_id=item_info.item_id AND requisition_details.req_id='$req_id' AND requisition_details.purchaser_id = '$pId' ";
